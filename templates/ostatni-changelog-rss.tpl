...
{if $smarty.foreach.smycka1.index < 10}
  <rdf:li resource="http://{$smarty.server.SERVER_NAME}/changelog.html#{$zmena.revision}" />
{/if}
{/foreach}
 </rdf:Seq>
</items>
...
{if $smarty.foreach.smycka2.index < 10}
  <item rdf:about="http://{$smarty.server.SERVER_NAME}/changelog.html#{$zmena.revision}">
    <title>r{$zmena.revision}</title>
    <link>http://{$smarty.server.SERVER_NAME}/changelog.html#{$zmena.revision}</link>
    <description>{$zmena.description}</description>
    <dc:creator>{$zmena.author}</dc:creator>

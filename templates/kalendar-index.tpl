    	...
    	{elseif $month[week][day]->isSelected()}
    	   ...
    	   <tr><td class="kal_week_day wd_{$smarty.section.day.index}">{$dny_zkratky[$smarty.section.day.index]}</td><td class="kal_week_day"><strong class="cislodne">{$cislodne}</strong></td><td class="den udalost{if $cislodne==$dnesek} dnesni{/if}">
            ...
            {assign var=payload value=$month[week][day]->getEntry()}
             <li class="vevent"><a class="url" href="udalost-{$payload.id}.html" title="{$payload.title}" {if isset($smarty.session.uzivatel.login) and $payload.vlozil==$smarty.session.uzivatel.login}class="edit"{/if}>{$payload.title|truncate:80:"...":false|escape}</a><span class="skryte"><span class="summary">{$payload.title|escape}</span><span class="description">{$payload.desc|escape}</span><abbr class="dtstart" title="{$payload.start_ical|escape}">Začátek: {$payload.start_hr|escape}</abbr><abbr class="dtend" title="{$payload.end_ical|escape}">Konec: {$payload.end_hr|escape}</abbr><div class="location">{$payload.misto|escape}</div></span></li>
            {/section}
            </ul>
					{else}&nbsp;{/if}
</td></tr>
...
<p><a href="https://chrome.google.com/extensions/detail/dojgpamkkdlccihbhgnflimdjkigimad">{obrazek soubor="browser-chrome.png" popisek="Google Chrome"}</a>Používáš Google Chrome? Vyzkoušej <a href="https://chrome.google.com/extensions/detail/dojgpamkkdlccihbhgnflimdjkigimad" class="external">rozšíření pro kalendář</a> do prohlížeče.</p>
{/if}

{if preg_match('/.*opera.*/i',$smarty.server.HTTP_USER_AGENT)}
<div class="cleaner">&nbsp;</div>
<p><a href="https://addons.opera.com/addons/extensions/details/zonglovani/?display=cs">{obrazek soubor="browser-opera.png" popisek="Opera"}</a>Používáš Operu? Vyzkoušej <a href="https://addons.opera.com/addons/extensions/details/zonglovani/?display=cs" class="external">rozšíření pro kalendář</a> do prohlížeče.</p>
{/if}
<p>

...
<li class="rss_{$id}"><a href="{$rss.url|escape}" title="Stránka {$rss.popis|escape}"{if !preg_match('/^http:\/\/zonglovani.info/',$rss.url)} class="external" rel="nofollow" onclick="pageTracker._trackPageview('/goto/{$rss.url|replace:'http://':''|regex_replace:"/^www\./":""|escape}');"{/if}>{$rss.popis|escape}</a></li>
{/foreach}
</ul>
{/if}

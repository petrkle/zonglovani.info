...
if($uzivatel_props['login']=='pek'){
	if(!isset($_SESSION) or (isset($_SESSION['uzivatel']['login']) and $_SESSION['uzivatel']['login']!='pek')){
...
$mojepozice=array_search($id,$pozice);
	...
	if(isset($uzivatel_props['foto'])){
		$smarty->assign('nahled','https://'.$_SERVER['SERVER_NAME'].LIDE_URL.'foto/'.$id.'.jpg');
	}else{
		$smarty->assign('nahled','https://'.$_SERVER['SERVER_NAME'].LIDE_URL.'m/'.$id.'.png');
	}

<?php
require("hlavicka.inc");
$title="Ohnivé ku¾ely";
hlavicka($title,__FILE__);
?>


<?php
require("titulek.inc");
titulek(__FILE__);
?>

<div id="stranka">

<div id="ramecek">


<div id="obsah">
<h1><?=$title?></h1>

<p>
<?=img("kuzely-fire.png","Pochodeò");?>
Ohnivé ku¾ely mají místo ¹ir¹í èásti hliníkovou trubku. Na ní je namotaný knot z kevlaru. Aby ku¾el hoøel, je potøeba nalít na knot <a href="#palivo">nìco hoølavého</a>.
</p>

<h3>Bezpeènostní pravidla</h3>
<p>
<ol>
<li>®ongluj na volném prostranství kde nemù¾e¹ nic zapálit.</li>
<li>Obleè se do star¹ího obleèení z bavlny. Bavlna je dùle¾itá, proto¾e nevzplane snadno jako látky z umìlé hmoty.</li>
<li>Mìj pøi ruce kbelík èisté studené vody. Na ochlazení pøípadných spálenin nebo ha¹ení.</li>
<li>Hoølaviny nech v originálním obalu v dostateèné vzdálenosti od místa ¾onglování.</li>
<li>®ongluj v dostateèné vzdálenosti od divákù.</li>
</ol>
</p>

<p>
Pøiprav si také vlhký bavlnìný hadr nebo azbestovou deku. Do nich zabalí¹ ku¾ely aby zhasly. A úplnì nejlep¹í je mít i hasicí pøístroj.
</p>

<h3>Trénink</h3>

<p>
Ohnivé ku¾ely jsou trochu jiné ne¾ obyèejné ku¾ely. Proto nejdøív trénuj za svìtla s nezapálenými ku¾ely. Kdy¾ získá¹ jistotu, mù¾e¹ zaèít trénovat za svìtla se zapálenými ku¾ely.
</p>
<p>
U ohnivých ku¾elù je trénink velmi dùle¾itý. Ka¾dé záváhání a spadnutí ku¾elu pøímo bije do oèí. ®onglování s ohnìm pak pøichází o èást svého kouzla. Navíc se mù¾e¹ i spálit.
</p>

<p>
Tak¾e trénuj za svìtla s nezapálenými ku¾ely. Pak za svìtla se zapálenými ku¾ely. A za tmy s ohnìm provádìj jen triky které má¹ plnì zvládnuté.
</p>

<a name="palivo" id="palivo"><h3>Palivo</h3></a>
<p>
Jsou vlastnì dvì mo¾nosti jakou hoølavinu pou¾ít.
<ol>
<li>petrolej</li>
<li>lampový olej</li>
</ol>
</p>
<p>
Osobnì dávám pøednost petroleji. Bývá levnìj¹í a ménì zapáchá. Na druhou stranu pøi hoøení vzniká více dýmu. Pokud pou¾ije¹ lampový olej tak si vyber bezbarvý. Bez pøidaných vùní.
</p>

<p>
<strong>NIKDY NEPO®ÍVEJ</strong> benzín, aceton nebo øedila. Jsou to hoølaviny I. tøídy nebezpeènosti. Hoøí velmi rychle a jejich páry jsou výbu¹né !!!
</p>

<h3>Zapaléní ku¾elù</h3>
<p>
<?=img("kuzely-fire-fill.png","Polití hoølavinou");?>
Hoølavinu nalij na knot ku¾elu nad nádobou. Je lep¹í nalít ménì hoølaviny, tak aby z knotu nezaèala vytékat. Kdy¾ pøelije¹ bude ti pøi ¾onglování z ku¾ele pøebyteèná hoølavina vyletovat.
</p>
<p>
Pøed zapálením odejdi s ku¾elem nejménì 5 metrù od nádoby s hoølavinou.
</p>  
<p>
<?=img("kuzely-fire-light.png","Zapálení");?>
Pøi zapalování dr¾ ku¾el knotem nahoru. Zapalovaèem zapal první ku¾el, od kterého pak zapálí¹ ostatní ku¾ely.
</p>

<p>
<?=img("kuzely-firea.png","Pochodeò");?>
Hoøící ku¾el dr¾ knotem nahoru.
</p>

<h3>Uha¹ení ku¾elù</h3>
<p>
Ku¾el pøikryj vlhkým bavlnìným hadrem a poèkej a¾ uhasne. S hadrem nemávej, nech ho na ku¾elu le¾et aby k ohni nemìl pøístup vzduch.
</p>

<h3>®onglování s ohnìm</h3>

<p>
<a href="/img/fire-1.jpg"><?=img("fire-1-tn.jpg","®onglování s ohnìm");?></a>
Za tmy s ohnìm provádìj jen triky které má¹ plnì zvládnuté.
</p>

<p>
Ohnivé ku¾ely nejsou stavìné na nepøetr¾ité hoøení. Mezi jednotlivýmí zapálení ku¾elù si dej chvíli pøestávku. Ku¾ely vychladnou a zmen¹í se tak nebezpeèí popálení.
</p>

<p>
<a href="/img/fire-2.jpg"><?=img("fire-2-tn.jpg","®onglování s ohnìm");?></a>
</p>

<p>
Pokroèilí ¾ongléøi mù¾ou s ohnìm i <a href="/kuzely/passing/" title="®onglování více lidí">passovat</a>.
</p>

<a href="#nahore" title="Pøesun na zaèátek stránky." class="nahoru">nahoru&nbsp;^^</a>

</div>




<div id="menu">

<?php
require("menu.inc");
menu(__FILE__);
?>


</div>


</div>
<div class="spacer"></div>
</div>

<?php
require("paticka.inc");
paticka($title);
?>

<!-- start -->
<div class="reklama">

<!--WZ-REKLAMA-1.0-STRICT-->

</div>
<!-- stop -->

</body>
</html>
